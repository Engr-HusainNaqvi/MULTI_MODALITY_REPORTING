...
st.title("🖼️ Upload a Chest X-ray to Generate Report")
...
def load_models():
    ...
    llama = AutoModelForCausalLM.from_pretrained("TinyLlama/TinyLlama-1.1B-Chat-v1.0").to(device)

    gpt2_tok = GPT2Tokenizer.from_pretrained("distilgpt2")
    gpt2 = GPT2LMHeadModel.from_pretrained("distilgpt2").to(device)

    return clip_model, clip_proc, blip_model, blip_proc, llama_tok, llama, gpt2_tok, gpt2
...
if uploaded_file:
    ...
    # Step 2: CLIP Retrieval (clinical findings)
    reference_findings = [
    ...
    # Step 3: Generate Draft with TinyLlama
    llama_prompt = (
        f"🧠 Caption: {caption}\n"
        f"📚 Retrieved Reports:\n" +
    ...
    # Step 4: Refine with GPT-2
    gpt2_input = (
        f"Caption: {caption}\n"
        f"Draft: {draft_report}\n"
        f"Refine this into a structured radiology report:"
    )
    gpt2_inputs = gpt2_tok(gpt2_input, return_tensors="pt").to(device)
    gpt2_output = gpt2_model.generate(**gpt2_inputs, max_new_tokens=200, do_sample=True)
    final_report = gpt2_tok.decode(gpt2_output[0], skip_special_tokens=True)
...
{final_report}
"""
    st.text_area("📄 Full Structured Report", report_output, height=500)
