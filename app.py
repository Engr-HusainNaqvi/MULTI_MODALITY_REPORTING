...
if uploaded_file:
    ...
    # 🔹 Step 1: Generate Caption via BLIP
    ...
    # 🔹 Step 2: Retrieve Matching Clinical Sentences via CLIP
    reference_findings = [
    ...
    # 🔹 Step 4: Refine via GPT-2
    gpt2_prompt = (
        f"Caption: {caption}\n"
        f"Draft: {draft_report}\n"
        f"Refine this into a structured radiology report:"
    )
    gpt2_inputs = gpt2_tok(gpt2_prompt, return_tensors="pt").to(device)
    gpt2_output = gpt2_model.generate(**gpt2_inputs, max_new_tokens=200)
...
{final_report}
"""
    st.text_area("📄 Complete Structured Report", report_output, height=600)
