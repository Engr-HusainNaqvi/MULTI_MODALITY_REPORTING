...
def main():
    ...
    img_inputs = clip_processor(images=img, return_tensors="pt").to("cuda")
    with torch.no_grad():
    ...
    # Text encoding
    text_inputs = clip_processor(text=retrieved_reports, return_tensors="pt", padding=True).to("cuda")
    with torch.no_grad():
        text_features = clip_model.get_text_features(**text_inputs)
    text_features = torch.nn.functional.normalize(text_features, dim=-1)

    # Cosine similarity to retrieve best matches
    ...
    blip_inputs = blip_processor(images=img, return_tensors="pt").to("cuda")
    ...
    falcon_inputs = falcon_tok(draft_prompt, return_tensors="pt").to(falcon_model.device)
    ...
    gpt2_model = GPT2LMHeadModel.from_pretrained("distilgpt2").cuda()
    ...
    gpt2_inputs = gpt2_tok(final_prompt, return_tensors="pt").to("cuda")
